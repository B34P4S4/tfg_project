def build_prompt(code, language):
    return f"""
Analiza el siguiente código fuente en {language} y determina si contiene vulnerabilidades de seguridad.
...
- "3.2": código CAPEC principal en formato numérico (ej: 89).
- "3.3": código CWE principal en formato numérico (ej: 89).
...
- "3.4" y "3.5" no pueden estar vacíos si se detecta vulnerabilidad. Debes inferir las técnicas MITRE ATT&CK más probables.
